...
from kyotei.storage import PredictionLogStore
...
def test_compare_logged_predictions_resolves_finished_race(tmp_path):
    ...
    rows = list(compare_logged_predictions(_FakeClient(), store, "20260731"))
    assert len(rows) == 1
    row = rows[0]
...
def test_compare_logged_predictions_unresolved_when_no_result(tmp_path):
    ...
    rows = list(compare_logged_predictions(_FakeClient(), store, "20260801"))
    assert len(rows) == 1
...
def test_compare_logged_predictions_empty_for_date_with_no_logs(tmp_path):
    store = PredictionLogStore(db_path=tmp_path / "test.db")
    rows = list(compare_logged_predictions(_FakeClient(), store, "20260101"))
    assert rows == []
