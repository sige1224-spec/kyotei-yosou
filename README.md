...
# 指定日の全レースをまとめて予想し、予想ログに記録する（前日以降に翌日分をまとめて仕込む用途）
kyotei predict-all --date 20260803 --venues all --races 1-12

# お気に入り競艇場の今日の開催・お気に入り選手の出走有無をまとめて表示
kyotei favorite today --date 20260802
```

`kyotei today` と `kyotei predict-all` を組み合わせ、`scripts/daily_routine.ps1` から
「前日分レビュー→翌日分の全レース予想」を自動実行できる。Windowsタスクスケジューラに
タスク `KyoteiDailyRoutine`（毎日20:30起動）として登録済み（ローカルでCLIを実行するだけ
...
- 「今日のお気に入り」ページ: アプリを開いたときに最初に見るホーム画面。お気に入り登録した
  競艇場で開催中のレースと、その中にお気に入り選手が出走しているかを一覧表示する。
- 「レース予想」ページ: 競艇場・開催日・レース番号を選んで予想を表示（競艇場・レース番号は
  複数選択可。複数選ぶと選択件数分の予想を折りたたみ表示でまとめて確認できる）。推定勝率のグラフ、
...
- 「勝ちパターン分析」ページ: backtestで蓄積したデータから、場ごとの的中率・回収率、
  予想1位レーンの推定勝率帯ごとの実際の的中率、天候・風速帯・波高帯ごとの的中率・回収率
  （2026-08-03以降に記録されたレース分）を確認できる。モデルの自信度（推定勝率）が実際の
  的中率と比例しているか、荒天時に的中率が下がる傾向があるか、の目安になる。
...
- `src/kyotei/backtest.py` / `src/kyotei/storage.py`（`BacktestStore`） — 過去レースを
  ...
  場ごと・自信度ごとの「勝ちパターン分析」、`stats_by_weather`/`stats_by_wind_speed`/
  `stats_by_wave_height`で天候・風速・波高ごとの的中率も分析できる（天候はスコアには
  未反映で記録・分析用途のみ）。
- `src/kyotei/favoritesview.py` — お気に入り競艇場で開催中のレースと、お気に入り選手の
  出走有無をまとめる（`kyotei favorite today` / Webの「今日のお気に入り」ホーム画面）。
