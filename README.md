...
- 「レース予想」ページ: 競艇場・開催日・レース番号を選んで予想を表示。推定勝率のグラフ、
  選手情報（級別・全国/当地勝率・モーター/ボート2連率・F数・平均ST、公式プロフィールへの
  リンクつき）、買い目候補（3連単を本命/中穴/大穴のタブで分類、2連単）、予算を入力しての
  金額配分、直前情報（展示タイム・進入コース・気象情報）、同じ会場の当日それまでのレース
  結果（公式サイトへのリンクつき）を確認できる。
