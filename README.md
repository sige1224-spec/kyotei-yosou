...
統計・ルールベースのロジックでレーンごとの推定勝率を算出する。
過去レースの答え合わせを蓄積し、的中率を検証する仕組みも備える。
...
- 「レース予想」ページ: 競艇場・開催日・レース番号を選んで予想を表示。推定勝率のグラフ、
  直前情報（展示タイム・進入コース・気象情報）も確認できる。
- 「検証ダッシュボード」ページ: backtestを画面から実行でき、日次の的中率推移グラフと
  直近の結果一覧を確認できる。

## 仕組み

- `src/kyotei/scraper/` — boatrace.jp から出走表・直前情報・結果ページを取得・パース。
  取得したHTMLは `data/cache/` にキャッシュし、公式サイトへの負荷を抑える。
...
- `src/kyotei/backtest.py` / `src/kyotei/storage.py` — 過去レースを出走表データの時点で
  予想し、実際の結果と突き合わせて `data/kyotei.db`（SQLite）に記録・集計する仕組み。
- `web/app.py` — Streamlit製のダッシュボード（予想画面・検証画面）。

予想結果はあくまで統計的な参考情報であり、的中を保証するものではない。
